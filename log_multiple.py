...
if __name__ == '__main__':
    dates = [str(day) for day in (start + timedelta(days=num) for num in range(0, (end-start).days + 1)) if day.isoweekday() <=5]

    for date in dates:
        request = log_hours(date=date, url=url, project_name=project_name, project_tags=project_tags, auth_token=auth_token)
